...
const CustomCalendar = ({
  ...
  const handleOutsideClick = (e: MouseEvent) => {
    if (close.current && !close.current.contains(e.target as Node)) {
      closeCalendar ? closeCalendar() : setCalendarOpen(false);
    }
  };
  useEffect(() => {
  ...
  return (
    <div
      ref={close}
      className="mx-auto rounded mt-2 w-[400px] h-[350px] p-2 bg-gray-200 flex flex-col relative "
    >
      <div className="flex flex-row justify-between">
        {/* <button className="m-2">Clear</button> */}
        <button
          className="m-2 flex-1 bg-blue-500 text-white rounded hover:bg-white hover:text-black"
      ...
      <div>
        {showDays && (
          <div>
            {!showMonth && !showYear && (
              <div className="mx-1 mt-3 bg-gray-50 grid grid-cols-7 text-center">
        ...
        {showMonth && (
          <Month
            ...
            minMonth={minMonth ? minMonth : undefined}
            currentYear={selectedDate.year()}
            minYear={minDate ? (minYear ? minYear : undefined) : undefined}
          />
        )}
        {showYear && (
          <Year
